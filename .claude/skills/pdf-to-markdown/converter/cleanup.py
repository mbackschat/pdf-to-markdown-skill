...
from .text import (
    ...
    extract_contents_entries_from_text,
...
def remove_running_headers(md_text: str) -> str:
    ...
    running_headers = {text for text, count in __import__("collections").Counter(heading_texts).items() if count >= 3}
...
def convert_contents_tables_to_lists(md_text: str) -> str:
    ...
    while i < len(lines):
        ...
        for row in rows:
            ...
            pairs = zip(text_parts, page_parts) if page_parts and len(page_parts) == len(text_parts) else ((text, "") for text in text_parts)
